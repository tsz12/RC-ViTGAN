...
def train(P,opt,train_fn,models,optimizers,train_loader,target_loader,pair_loader,logger):
    #print(f"成功进入train函数")
    generator,g_ema=models
    opt_G=optimizers

    losses={'G_train1_loss':[]}
    ...
    #-----检查一下数据是否加载成功---------
        ...
        gen_images = _sample_generator(generator, images.size(0),enable_grad=True,imgs=images)
        #print(f"成功产生图片")
        g_loss = train_fn["train1"](P, opt, target_images,gen_images)
...
def worker(P):
    ...
    train_set, _, target_set,pair_set,image_size = get_dataset(dataset=options['dataset'])
    ...
    generator = vit_my(patch_size=16)
    g_ema = vit_my(patch_size=16)
    print(f"模型加载成功")
    # #-----加载dino的预训练参数---------
    ...
    #     #print(f"层名分别为{name}")
    #     if "convs" in name or "to_rgb" in name or "norm." in name or "style" in name or "mlp_gamma" in name or "mlp_beta" in name:
    #         param.requires_grad=True
    #     else:
    #         param.requires_grad=False
    # for name, param in generator.named_parameters():
    #     if param.requires_grad:
    #         print(f"允许更新的参数为{name}")
    ...
    train(P,options,P.train_fn,
            ...
            train_loader=train_loader,target_loader=target_loader,pair_loader=pair_loader,logger=logger)
