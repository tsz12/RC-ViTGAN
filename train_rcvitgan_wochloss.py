...
writer=SummaryWriter('out/log_dino_noSLN_smoothL1_wochloss')#可视化数据放在这个文件夹
...
def train(P, opt, train_fn, models, optimizers, 
    ...
    for step in range(P.starting_step, opt['max_steps'] + 1):
        ...
        #有标签的原图、有标签的目标图、无标签的原图各自对应的224(gray)和128(color)
        # ltrain_gray_images, ltrain_color_images = next(ltrain_pair_loader)
        # ltrain_gray_images=ltrain_gray_images.cuda()
        # ltrain_color_images=ltrain_color_images.cuda()

        # ltarget_gray_images, ltarget_color_images = next(ltarget_pair_loader)
        # ltarget_gray_images=ltarget_gray_images.cuda()
        # ltarget_color_images=ltarget_color_images.cuda()

        # utrain_gray_images, utrain_color_images = next(utrain_pair_loader)
        # utrain_gray_images=utrain_gray_images.cuda()#
        # utrain_color_images=utrain_color_images.cuda()#
        ...
        # set_grad(generator, False)#误差反向传播，不计算梯度
        # #条件图片和生成图片对
        ...
        #     writer.add_scalars('stage2_DS',{'ds_loss': losses['DS_loss'][-1], 'ds_penalty_loss': losses['DS_penalty'][-1]}, step)
        # if step % 2 == 0:
        #     # Start dp training
        ...
        #     loss = dp_loss+dp_aux['penalty']
        #     loss.backward()
        #     # dp_loss.backward()
        #     # dp_aux['penalty'].backward()
        #     # loss = dp_loss+dp_aux['penalty']
        #     opt_DP.step()
        ...
        gp_loss ,gp_aux= train_fn["train3_G_wochloss_match"](P, discriminator_pair, opt,lgen_images,ltarget_images,u_img_input)#,ugen_images)
        gs_loss ,gs_aux= train_fn["train3_G_wochloss_match"](P, discriminator_single, opt,ugen_images,target_images,ugen_images)
        ...
        if step % P.evaluate_every == 0:
            ...
            torch.save(G_state_dict, logger.logdir + '/gen_stage3_wochloss.pt')
            torch.save(DP_state_dict, logger.logdir + '/disP_stage3_wochloss.pt')
            torch.save(DS_state_dict, logger.logdir + '/disS_stage3_wochloss.pt')
            torch.save(Ge_state_dict, logger.logdir + '/gen_ema_stage3_wochloss.pt')
            ...
            if step % P.save_every == 0:
                torch.save(G_state_dict, logger.logdir + f'/gen_stage3_wochloss_{step}.pt')
                torch.save(DP_state_dict, logger.logdir +f'/disP_stage3_wochloss_{step}.pt')
                torch.save(DS_state_dict, logger.logdir + f'/disS_stage3_wochloss_{step}.pt')
                torch.save(Ge_state_dict, logger.logdir + f'/gen_ema_stage3_wochloss_{step}.pt')
                torch.save({'epoch': step,'optim_G': opt_G.state_dict(),'optim_DP': opt_DP.state_dict(),'optim_DS': opt_DS.state_dict(),
            }, logger.logdir + f'/optim_stage3_wochloss_{step}.pt')
            ...
            }, logger.logdir + '/optim_stage3_wochloss.pt')
