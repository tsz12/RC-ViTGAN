...
def train(P, opt, train_fn, models, optimizers, 
    ...
    for step in range(P.starting_step, opt['max_steps'] + 1):
        ...
        if step % P.evaluate_every == 0:
            ...
            torch.save(G_state_dict, logger.logdir + '/gen_stage3.pt')
            torch.save(DP_state_dict, logger.logdir + '/disP_stage3.pt')
            torch.save(DS_state_dict, logger.logdir + '/disS_stage3.pt')
            torch.save(Ge_state_dict, logger.logdir + '/gen_ema_stage3.pt')
            ...
            if step % P.save_every == 0:
                torch.save(G_state_dict, logger.logdir + f'/gen_stage3_{step}.pt')
                torch.save(DP_state_dict, logger.logdir +f'/disP_stage3_{step}.pt')
                torch.save(DS_state_dict, logger.logdir + f'/disS_stage3_{step}.pt')
                torch.save(Ge_state_dict, logger.logdir + f'/gen_ema_stage3_{step}.pt')
                torch.save({'epoch': step,'optim_G': opt_G.state_dict(),'optim_DP': opt_DP.state_dict(),'optim_DS': opt_DS.state_dict(),
            }, logger.logdir + f'/optim_stage3_{step}.pt')
