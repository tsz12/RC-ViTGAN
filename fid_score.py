...
def compute_stats_from_dataloader(label_loader,origin_loader,model):
...
def compute_stats_from_my_G(G, model, size, batch_size,pair_loader):
    model.eval()
...
def my_fid_score(path_base, G, size=9668, batch_size=32, model=None, dims=192):
    """Calculates the FID between G and a pre-computed stats"""
    if not os.path.exists(path_base):
        raise RuntimeError('Invalid path: %s' % path_base)
    ...
    pair_set,resolution= get_dataset(dataset='val_data')
    pair_loader = DataLoader(pair_set, shuffle=False, pin_memory=False, num_workers=8,
...
def my_precompute_stats(save_path, model=None, dims=192): 
    label_set=get_dataset(dataset='label_fid')
    origin_set=get_dataset(dataset='original_fid')
    print(f"label_set的大小为{len(label_set)}")
    print(f"origin_set的大小为{len(origin_set)}")
    #data=torch.cat([label_set,origin_set],dim=0)
    #print(f"data的大小为{len(data)}")
    label_loader=DataLoader(label_set, shuffle=False, pin_memory=True, num_workers=8,
                                          batch_size=32, drop_last=False)
    origin_loader=DataLoader(origin_set, shuffle=False, pin_memory=True, num_workers=8,
                                          batch_size=32, drop_last=False)  
    #dataLoader=DataLoader(data, shuffle=False, pin_memory=True, num_workers=8,
    #                                       batch_size=32, drop_last=True)  

    if model is None:
        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
        print(f"block_idx为{block_idx}")
...
if __name__ == '__main__':
    ...
    my_precompute_stats(save_path='base_stats.npz')

    fid_value=my_fid_score(path_base='base_stats.npz', G=generator, size=9546, batch_size=16, model=None, dims=192)
    print(f"fid_value为{fid_value}")
    # label_set=get_dataset(dataset='label_fid')
    # origin_set=get_dataset(dataset='original_fid')
