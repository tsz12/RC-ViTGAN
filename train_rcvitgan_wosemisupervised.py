...
writer=SummaryWriter('out/log_dino_noSLN_smoothL1_wosemisupervised')#可视化数据放在这个文件夹
...
def train(P, opt, train_fn, models, optimizers, 
    ...
    for step in range(P.starting_step, opt['max_steps'] + 1):
        ...
        gs_loss ,gs_aux= train_fn["train3_G_match"](P, discriminator_single, opt,ugen_images,ugen_images,ugen_images)#这里相当于targetloss为0，伪标签没有发挥作用
        ...
        if step % P.evaluate_every == 0:
            ...
            torch.save(G_state_dict, logger.logdir + '/gen_stage3_woss.pt')
            torch.save(DP_state_dict, logger.logdir + '/disP_stage3_woss.pt')
            torch.save(DS_state_dict, logger.logdir + '/disS_stage3_woss.pt')
            torch.save(Ge_state_dict, logger.logdir + '/gen_ema_stage3_woss.pt')
            ...
            if step % P.save_every == 0:
                torch.save(G_state_dict, logger.logdir + f'/gen_stage3_woss_{step}.pt')
                torch.save(DP_state_dict, logger.logdir +f'/disP_stage3_woss_{step}.pt')
                torch.save(DS_state_dict, logger.logdir + f'/disS_stage3_woss_{step}.pt')
                torch.save(Ge_state_dict, logger.logdir + f'/gen_ema_stage3_woss_{step}.pt')
                torch.save({'epoch': step,'optim_G': opt_G.state_dict(),'optim_DP': opt_DP.state_dict(),'optim_DS': opt_DS.state_dict(),
            }, logger.logdir + f'/optim_stage3_woss_{step}.pt')
            ...
            }, logger.logdir + '/optim_stage3_woss.pt')
