...
from stylegan2.layers import PixelNorm, Upsample, Blur
...
class ModulatedConv2d(nn.Module):#在Style里使用，应该就是这里出错的，把这里看懂再去那里看看
    def __init__(self, in_channel, out_channel, kernel_size, style_dim,
        ...
        if upsample:
            factor = 2
            p = (len(blur_kernel) - factor) - (kernel_size - 1)#0
            pad0 = (p + 1) // 2 + factor - 1#1
            pad1 = p // 2 + 1#1
    ...
    def forward(self, input, style):
        batch, in_channel, height, width = input.shape#这里的in_channel应该等于embed_dim
        #print(f"input的大小为{input.shape}")#torch.Size([2, 384, 8, 8])这里还是正常的([2, 768, 16, 16])
        ...
        )#self.kernel_size为1，这里用1*1的卷积来改变张量的通道数
        #input = input.view(1, batch * in_channel, height, width)
        input = input.reshape(1, batch * in_channel, height, width)#torch.Size([1, 768, 8, 8])这里还是对的
        #print(f"input的大小为{input.shape}")
        if self.upsample:
            #将权重进行调制之后用于转置卷积实现上采样，然后采用Blur对图片进行滤波模糊操作，有利于降噪
            weight = weight.view(
                batch, self.out_channel, in_channel, self.kernel_size, self.kernel_size
            )
            ...
            #print(f"out.shape为{out.shape}")
            out = out.view(batch, self.out_channel, height, width)
            #print(f"输入self.blur的大小为{out.shape}")
            out = self.blur(out)
        else:
...
class NoiseInjection(nn.Module):
    ...
    def forward(self, image, noise=None):
        if noise is None:
            ...
            noise = image.new_empty(batch, 1, height, width).normal_()#返回与image形状相同的未初始化张量，并对其进行归一化
...
class StyleLayer(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, style_dim,
                 upsample=False, blur_kernel=[1, 3, 3, 1], demodulate=True,noise_injection=True):#blur_kernel是用于图像增强的参数
        super().__init__()
        #print(f"当前StyleLater的in_channel为{in_channel},out_channel为{out_channel}")
    ...
    def forward(self, input, style, noise=None):
        ...
        #print(f"StyleLayer后x的大小为{out.shape}")
        return out
...
#应该是这里修改的问题，不然不会copy_vit_generator也出问题
    ...
    def __init__(self, size,
        ...
        if small32:
            ...
            self.channels = {
                ...
                16: 512,
                32: 512,
                64: int(256 * channel_multiplier),#256*2
                128: int(128 * channel_multiplier),#此时的self.channels应该是256
                256: int(64 * channel_multiplier),#128
                512: int(32 * channel_multiplier),#64
                1024: int(16 * channel_multiplier),#32
            }
        ...
        for i in range(3, self.log_size + 1):#通道数从embed_dim出发逐渐增加，又逐渐减小
